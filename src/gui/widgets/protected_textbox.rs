...
use druid::text::{EditableText, ImeInvalidation, Selection, TextComponent, TextStorage};
use druid::{BoxConstraints, Color, Command, Env, Event, EventCtx, FontDescriptor, HotKey, Insets, KeyEvent, KeyOrValue, LayoutCtx, LifeCycle, LifeCycleCtx, PaintCtx, Point, Rect, RenderContext, Size, SysMods, TextAlignment, theme, TimerToken, UpdateCtx, Vec2, Widget};
use druid::debug_state::DebugState;
use druid::piet::TextLayout as PietTextLayout;
...
impl<T: EditableText + TextStorage> ProtectedTextBox<T> {
...
impl<T> ProtectedTextBox<T> {
    ...
    pub fn set_text_size(&mut self, size: impl Into<KeyOrValue<f64>>) {
        if !self.text().can_write() {
            log::warn!("set_text_size called with IME lock held.");
            return;
        }
    ...
    pub fn set_font(&mut self, font: impl Into<KeyOrValue<FontDescriptor>>) {
        if !self.text().can_write() {
            log::warn!("set_font called with IME lock held.");
            return;
        }
        let font = font.into();
    ...
    pub fn set_text_alignment(&mut self, alignment: TextAlignment) {
        if !self.text().can_write() {
            log::warn!("set_text_alignment called with IME lock held.");
            return;
        }
    ...
    pub fn set_text_color(&mut self, color: impl Into<KeyOrValue<Color>>) {
        if !self.text().can_write() {
            log::warn!("set_text_color called with IME lock held.");
            return;
        }
...
impl<T: TextStorage + EditableText> ProtectedTextBox<T> {
...
impl<T: TextStorage + EditableText> Widget<T> for ProtectedTextBox<T> {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut T, env: &Env) {
        match event {
            ...
            if !self.text().is_composing()
                && ctx.is_focused()
                && cmd.is(druid::commands::SELECT_ALL) =>
                {
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &T, env: &Env) -> Size {
        if !self.text().can_write() {
            log::warn!("Widget::layout called with outstanding IME lock.");
        }
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &T, env: &Env) {
        if !self.text().can_read() {
            log::warn!("Widget::paint called with outstanding IME lock, skipping");
            return;
        }
        let size = ctx.size();
...
impl<T: TextStorage + EditableText> Default for ProtectedTextBox<T> {
    fn default() -> Self {
        ProtectedTextBox::new()
    }
}
